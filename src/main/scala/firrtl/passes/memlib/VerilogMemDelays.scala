...
class VerilogMemDelays extends Pass {
  ...
  override val prerequisites: Set[Class[Transform]] = firrtl.stage.Forms.LowForm + classOf[firrtl.passes.RemoveValidIf]

  override val dependents: Set[Class[Transform]] = Set(classOf[VerilogEmitter], classOf[SystemVerilogEmitter])
